...
#define VR_SAFE_UNREF(x)\
if (NULL!=(x)) {(x)->decRef();}
#define VR_SAFE_REF(x)\
if (NULL!=(x)) (x)->addRef();
#define VR_SAFE_ASSIGN(dst, src) \
{\
if (src!=NULL)\
{\
src->addRef();\
...
public:
    ...
    SharedPtr(const SharedPtr& o) : mT(o.mT) { VR_SAFE_REF(mT); }
    ~SharedPtr() { VR_SAFE_UNREF(mT); }
    
    SharedPtr& operator=(const SharedPtr& rp) {
        VR_SAFE_ASSIGN(mT, rp.mT);
        return *this;
    }
