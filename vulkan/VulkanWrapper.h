...
#ifdef VR_VULKAN_USE_LIB_WRAPPER
#define VK_NO_PROTOTYPES 1 // define it before including <vulkan/vulkan.h>
#endif
...
#define CALL_VK(func) \
{ \
auto restemp = (func); VR_ASSERT(restemp == VK_SUCCESS); \
}
// #define VR_VK_CHECK(res) if(res != VK_SUCCESS) VR_PRINT("Error code : %d\n", res);
#define VR_VK_CHECK VR_CHECK
#define VR_VK_LIB_CHECK(libmodule) if(libmodule == nullptr){ \
        VR_ERROR("Load vulkan library error\n"); \
        return false; \
    }
...
uint32_t GetInstanceVersion(void);
